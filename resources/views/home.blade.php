...
<div class="min-h-screen overflow-y-visible">
    ...
    <div x-data="{ openTask: null }">  
        ...
        <div class="py-8">
            ...
            <div @click="openTask = openTask === {{ $task->id }} ? null : {{ $task->id }}" 
                class="m-1 transition-all duration-300 ease-in-out" 
                :class="{'shadow-lg': openTask === {{ $task->id }}}">
                
                <div tabindex="0" class="bg-white border-t border-collapse p-3 flex justify-between items-center hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:rounded-lg transition-all duration-300 ease-in-out" 
                    :class="{'border-x rounded-t-md': openTask === {{ $task->id }}}">
